...
const ClassModal: React.FC<ClassModalProps> = ({
  ...
  const getHistoricData = () => {
    ...
    return {
      ...
      classFormats,
      teachers,
      peakParticipants: Math.max(...historicClasses.map(cls => cls.participants)),
  ...
  const handleSchedule = () => {
    ...
    const scheduledClass: ScheduledClass = {
      id: `${selectedSlot.day}-${selectedTime}-${selectedSlot.location}-${Date.now()}`,
      day: selectedSlot.day,
      time: selectedTime,
      location: selectedSlot.location,
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
      <div className={`${modalBg} rounded-2xl shadow-2xl max-w-7xl w-full m-4 max-h-[95vh] overflow-y-auto border ${borderColor}`}>
        <div className={`flex items-center justify-between p-6 border-b ${borderColor} bg-gradient-to-r from-purple-600/20 to-pink-600/20`}>
          <div>
            <h2 className={`text-2xl font-bold ${textPrimary}`}>Schedule Class</h2>
        ...
        <div className={`p-6 border-b ${borderColor}`}>
          <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 p-4 rounded-xl border border-blue-500/20">
            ...
            <div className="text-sm text-blue-200">
              {selectedSlot.location === 'Supreme HQ, Bandra' ? (
                <div>
                  <p>• <strong>Focus on:</strong> Fit, PowerCycle, and Barre classes</p>
                  <p>• <strong>PowerCycle classes ONLY allowed here</strong> (3 studios available)</p>
                  <p>• <strong>NOT allowed:</strong> Amped Up, HIIT</p>
                </div>
              ) : (
                <div>
                  <p>• <strong>All class formats allowed EXCEPT PowerCycle</strong></p>
                  <p>• <strong>PowerCycle classes can only be scheduled at Supreme HQ</strong></p>
                  <p>• Maximum 2 parallel classes (2 studios available)</p>
                </div>
              )}
            </div>
          </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {allTeachers.map(teacher => {
                      ...
                      const isNew = customTeachers.some(t => `${t.firstName} ${t.lastName}` === teacher && t.isNew);
                      return (
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                              rec.priority >= 8 ? 'bg-green-500/20 text-green-300' :
                              rec.priority >= 6 ? 'bg-yellow-500/20 text-yellow-300' :
            ...
            <div className="space-y-6">
              <div className="bg-gradient-to-r from-blue-500/20 to-cyan-500/20 p-6 rounded-xl border border-blue-500/30">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-green-500/10 rounded-lg">
                      <div className="text-sm text-green-300 mb-2">Best Performing Class</div>
                      <div className={`${textPrimary} font-medium`}>{historicData.bestClass}</div>
                      <div className={`text-sm ${textSecondary}`}>with {historicData.bestTeacher}</div>
                    </div>
                  </div>

                  <div className="space-y-4">
                    <div className="p-4 bg-purple-500/10 rounded-lg">
                      <div className="text-sm text-purple-300 mb-2">Popular Class Formats</div>
                      <div className="space-y-2">
                        {historicData.classFormats.slice(0, 3).map((format, index) => (
                    ...
                    <div className="p-4 bg-orange-500/10 rounded-lg">
                      <div className="text-sm text-orange-300 mb-2">Success Rate</div>
                      <div className="flex items-center">
                        <div className={`text-2xl font-bold ${textPrimary} mr-2`}>
                          {Math.round(historicData.successRate * 100)}%
                        </div>
                        <div className={`text-sm ${textSecondary}`}>
                          of classes exceed average attendance
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-6 p-4 bg-indigo-500/10 rounded-lg">
                  <div className="text-sm text-indigo-300 mb-2">Recommendations Based on Historic Data</div>
